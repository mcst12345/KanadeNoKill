...
class ThreadGroup implements Thread.UncaughtExceptionHandler {
    ...
    public boolean vmAllowSuspension;

    public int nUnstartedThreads = 0;
    public int nthreads;
    ...
    public ThreadGroup(Void unused, ThreadGroup parent, String name) {
        this.name = name;
     ...
     *                                thread in the specified thread group.
     */
    public static Void checkParentAccess(ThreadGroup parent) {
        parent.checkAccess();
        return null;
    }
    ...
    public int enumerate(Thread list[], int n, boolean recurse) {
    ...
    public int enumerate(ThreadGroup list[], int n, boolean recurse) {
        int ngroupsSnapshot = 0;
        ThreadGroup[] groupsSnapshot = null;
        synchronized (this) {
            if (destroyed) {
     ...
     */
    @SuppressWarnings("deprecation")
    public boolean stopOrSuspend(boolean suspend) {
     ...
     */
    public final void add(ThreadGroup g) {
        synchronized (this) {
            if (destroyed) {
                throw new IllegalThreadStateException();
            }
     ...
     */
    public void remove(ThreadGroup g) {
        synchronized (this) {
            if (destroyed) {
                return;
            }
