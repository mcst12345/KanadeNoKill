...
public class GameData {
    ...
    public static final BiConsumer<ResourceLocation, ForgeRegistry<?>> LOCK_VANILLA = (name, reg) -> reg.slaves.values().stream().filter(o -> o instanceof ILockableRegistry).forEach(o -> ((ILockableRegistry) o).lock());
    ...
    public static ForgeRegistry<EntityEntry> entityRegistry;
    ...
    public static Multimap<ResourceLocation, ResourceLocation> injectSnapshot(Map<ResourceLocation, ForgeRegistry.Snapshot> snapshot, boolean injectFrozenData, boolean isLocalWorld) {
        ...
        if (count > 0) {
            ...
            missing.entrySet().stream().filter(e -> !e.getValue().isEmpty()).forEach(m ->
            {
                ...
                List<MissingMappings.Mapping<?>> lst = event.getAllMappings().stream().filter(e -> e.getAction() == MissingMappings.Action.DEFAULT).sorted(Comparator.comparing((Function<MissingMappings.Mapping<?>, String>) Object::toString)).collect(Collectors.toList());
                if (!lst.isEmpty()) {
